...
let PenguCommand = class PenguCommand extends framework_1.Command {
    async run(message, args) {
        ...
        const { url } = await util_1.fetch("https://nekos.life/api/v2/img/tickle");
        if (!url)
        ...
        const embed = new discord_js_1.MessageEmbed()
            ...
            .setColor("RANDOM")
            .setImage(url);
        return message.channel.send(await message.fetchLanguageKey("commands/images:tickle.response", { from: `${message.author}`, to: `${mentioned}` }), { embed });
    }
};
